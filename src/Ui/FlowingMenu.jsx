...
function MenuItem({ link, text, image }) {
  ...
  const repeatedMarqueeContent = Array.from({ length: 8 }).map((_, idx) => (
    <React.Fragment key={idx}>
      <span className="whitespace-nowrap text-[#060606] uppercase font-normal text-[4vh] leading-[1.2] px-[1vw]">
        {text}
      </span>
      <div
        className="inline-block w-[12vh] h-[7vh] mx-[2vw] rounded-[50px] bg-cover bg-center shrink-0"
    ...
    <div className="flex-1 relative overflow-hidden text-center shadow-[0_-1px_0_0_#fff]" ref={itemRef}>
      ...
      >
        <div className="h-full flex items-center" ref={marqueeInnerRef}>
          <div className="flex items-center h-full whitespace-nowrap will-change-transform animate-marquee">
            {repeatedMarqueeContent}
          </div>
          <div className="flex items-center h-full whitespace-nowrap will-change-transform animate-marquee2">
            {repeatedMarqueeContent}
          </div>
        </div>
      </div>
